    ...
    for(var i = 0; i < styles.length; i++) {
      ...
      switch (prop) {
        ...
        case 'font-style':
          ...
          break;
        case 'color':
          if (!firepad.fontOptions.allowColor || styleEqual(val, this.firepadDefaultStyles.color))
            break;
        ...
        case 'background-color':
          if (!firepad.fontOptions.allowBackground || styleEqual(val, this.firepadDefaultStyles.backgroundColor))
            break;
        ...
        case 'text-align':
          ...
          break;
        case 'font-size':
          var allowedSizing = firepad.fontOptions.allowSize;
          var availableSizes = firepad.fontOptions.availableSizes instanceof Array && firepad.fontOptions.availableSizes.length;
          if (!allowedSizing || styleEqual(val, this.firepadDefaultStyles.fontSize))
            break;
          var size = null;
          var allowedValues = ['px','pt','%','em','xx-small','x-small','small','medium','large','x-large','xx-large','smaller','larger'];
          if (!availableSizes && firepad.utils.stringEndsWith(val, allowedValues)) {
            size = val;
          }
          else if (availableSizes && parseInt(val) in availableSizes) {
            size = parseInt(val)+'px';
          }
          size = !size && firepad.fontOptions.defaultSize ? parseInt(firepad.fontOptions.defaultSize) : false;
          if (size) {
            textFormatting = textFormatting.fontSize(size);
          }
          break;
        case 'font-family':
          if (!firepad.fontOptions.allowFamily || styleEqual(val, this.firepadDefaultStyles.fontFamily))
            break;
