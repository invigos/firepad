...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.replaceText = function(start, end, text, attributes, origin) {
    ...
    var from = this.posFromTextIndex(start);
    var to = typeof end === 'number' ? this.posFromTextIndex(end) : null;
    this.codeMirror.replaceRange(text, from, to, newOrigin);
  };
      ...
      while ( (pos = text.indexOf(EntitySentinelCharacter)) != -1 ) {
        entitySentinel = line.markedSpans.reduce(function( p, c ) {
          ...
          if (c.marker.replacedWith && c.marker.replacedWith.tagName.toLowerCase() == 'img') {
            ...
            return c.marker.replacedWith != null && (from + entitiesTextLength) == (pos + add) ? c : p;
          } else {
            return c.marker.replacedWith != null && (c.from + entitiesTextLength) == (pos + add) ? c : p;
          }
        }, null);
