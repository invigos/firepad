...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.clearDanglingFontSizeMarksFromLine_ = function(line, fsValue) {
    ...
    this.codeMirror.findMarksAt({line: line, ch: this.codeMirror.getLine(line).length + 1}).forEach(function(mark){
      ...
      if (!mark.className.match(className)) mark.clear();
    });
  };
  ...
  RichTextCodeMirror.prototype.onCodeMirrorPaste_ = function( cm, e ) {
    ...
    var html = e.clipboardData ? e.clipboardData.getData('text/html') || e.clipboardData.getData('text') : null;
  ...
  RichTextCodeMirror.prototype.updateCurrentAttributesFromLineAttributes_ = function() {
    ...
    //ch 0 contains previous line styles/sentinel, so check styles at ch 1
    var pos = this.codeMirror.indexFromPos({ line: head.line, ch: 1 });
    var spans = this.annotationList_.getAnnotatedSpansForPos(pos);
    this.currentAttributes_ = {};
