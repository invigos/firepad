...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.onCodeMirrorCopyCut_ = function( cm, e ) {
    ...
    fp.copyText = fp.lastCopyText = val;
    e.preventDefault();
  };
 ...
 var spans = this.annotationList_.getAnnotatedSpansForPos(head);
            if (spans[0]) {
                var elementEnd = spans[0].pos + spans[0].length;
                if (elementEnd === head) {
                    if (spans[0].annotation.attributes['quickmark']) {
                        var spans = this.annotationList_.getAnnotatedSpansForPos(head,true);
                    }
                }
            }
