...
firepad.fontOptions = {
    ...
    allowSize: true,
    defaultSize: 14,
    availableSizes: [14, 16, 16.01, 20, 25]
};
...
firepad.Firepad = (function (global) {
  ...
  Firepad.prototype.rangeHasAttributes = function(start, end) {
    ...
    Firepad.prototype.formatParagraph = function (size) {
        ...
        //no selection, only cursor - head and anchor are same objects
        //no need to keep Pos type for it, setSelection using duck typing
        if (head === anchor) {
            anchor = JSON.parse(JSON.stringify(head));
        }

        head.ch   = 0;
        anchor.ch = this.codeMirror_.doc.getLine(selection.anchor.line).length;

        //select complete lines and update them
