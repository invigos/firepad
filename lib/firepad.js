...
firepad.Firepad = (function (global) {
  ...
  Firepad.prototype.rangeHasAttributes = function(start, end) {
    ...
    Firepad.prototype.formatParagraph = function (size) {
        ...
        var selection = this.codeMirror_.doc.listSelections()[0];
        if (!selection) {
            var cursor = this.codeMirror_.doc.getCursor();
            ...
            selection = this.codeMirror_.doc.listSelections()[0];
        }
        ...
        head.ch   = 0;
        anchor.ch = this.codeMirror_.doc.getLine(selection.anchor.line).length;

        this.codeMirror_.doc.setSelection(head, anchor);
    ...
    Firepad.prototype.addToolbar_ = function () {
        ...
        var buttons = [
            ...
            { evt: 'cut' }, { evt: 'copy' }, { evt: 'paste' },

            { evt: 'format-paragraph' },

            { evt: 'undo' }, { evt: 'redo' },

            { evt: 'bold' }, { evt: 'italic' },

            { evt: 'font-size' }, { evt: 'font' }, { evt: 'color' },

            { evt: 'left', handler: 'align', args: [ 'left' ] },
            { evt: 'center', handler: 'align', args: [ 'center' ] },
            { evt: 'right', handler: 'align', args: [ 'right' ] },

            { evt: 'ordered-list' }, { evt: 'unordered-list' },

            { evt: 'todo-list' },

            { evt: 'indent-increase', handler: 'indent' }, { evt: 'indent-decrease', handler: 'unindent' },

            { evt: 'insert-image', handler: 'makeImageDialog_' }
        ];

        buttons.forEach(function( b ) {
            var evt = b.evt;
            var handler = b.handler ? b.handler : toCamelCase(b.evt);
            var args = [ this[ handler ] ];
