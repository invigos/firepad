...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.handleInitialRevisions_ = function() {
    ...
    // todo: this is here just for debug purposes
    //var ops = [], curOp = [];
    //this.document_.ops.forEach(function(op){
    //  if (op.attributes.l) {ops.push(curOp);curOp=[]}
    //  else {curOp.push({t:op.text,a:op.attributes})}
    //});
    //ops.forEach(function(curOp){
    //  console.log(curOp.reduce(function(p,c){p+=c.t+JSON.stringify(c.a);return p;},''))
    //});
  ...
  FirebaseAdapter.prototype.handlePendingReceivedRevisions_ = function() {
    //console.log(fp.firebaseAdapter_.document_.ops.length);
    //var rnd = Math.ceil(Math.random()*1000);
    //console.info('handlePendingReceivedRevisions_'); //console.trace();
    //console.group('handlePendingReceivedRevisions_'+rnd)
    ...
    //console.groupEnd('handlePendingReceivedRevisions_'+rnd);
  };
